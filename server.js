...
// Please use HTTPs on non-192.168.1.7 domains.
var isUseHTTPs = true;
...
function serverHandler(request, response) {
    try {
        ...
        if (fs.statSync(filename).isDirectory()) {
            ...
            if (filename.indexOf(resolveURL('/chat/MultiRTC/')) !== -1) {
                filename = filename.replace(resolveURL('/chat/MultiRTC/'), '');
                filename += resolveURL('/chat/MultiRTC/index.html');
            } else if (filename.indexOf(resolveURL('/chat/')) !== -1) {
                filename = filename.replace(resolveURL('/chat/'), '');
                filename += resolveURL('/chat/index.html');
            } else {
                filename += resolveURL('/chat/index.html');
            }
        }
        ...
        fs.readFile(filename, 'binary', function(err, file) {
            ...
            try {
                ...
                if (chat.length) {
                    ...
                    var otherchat = '<section class="experiment" id="chat"><details><summary style="text-align:center;">Check ' + (chat.length - 1) + ' other RTCMultiConnection-v3 chat</summary>' + h2 + '<ol>';
                    chat.forEach(function(f) {
                        if (f && f !== 'index.html' && f.indexOf('.html') !== -1) {
                            otherchat += '<li><a href="/chat/' + f + '">' + f + '</a> (<a href="https://github.com/muaz-khan/RTCMultiConnection/tree/master/chat/' + f + '">Source</a>)</li>';
                        }
                    });
                    otherchat += '<ol></details></section><section class="experiment own-widgets latest-commits">';

                    file = file.replace('<section class="experiment own-widgets latest-commits">', otherchat);
                }
            } catch (e) {}
...
function runServer() {
    ...
    app = app.listen(port, process.env.IP || '0.0.0.0', function(error) {
        ...
        if (addr.address != '192.168.1.7' && !isUseHTTPs) {
            console.log('Warning:');
            console.log('\x1b[31m%s\x1b[0m ', 'Please set isUseHTTPs=true to make sure audio,video and screen chat can work on Google Chrome as well.');
        }
